    ...
    <div class="max-w-[1200px] mx-auto px-8 pb-16">
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div class="panel lg:col-span-3 flex flex-col h-[calc(100vh-220px)] overflow-hidden">
          ...
          <div ref="container" class="flex-1 overflow-y-auto p-4 bg-ink-950/[0.03] font-mono text-xs leading-relaxed text-ink-900">
            <div v-for="(line, i) in output" :key="i" class="whitespace-pre-wrap">
              <div v-if="line.type === 'cmd'" class="text-accent font-semibold mt-2">{{ line.text }}</div>
              <div v-else>{{ line.text }}</div>
            </div>
            <div v-if="output.length === 0" class="text-ink-500">// 在下方输入命令开始执行…</div>
          </div>

          <div class="border-t border-canvas-300 p-3 flex items-center gap-2 bg-canvas-50">
            ...
            <input
              ...
              class="flex-1 bg-transparent text-sm font-mono text-ink-900 placeholder:text-ink-500 focus:outline-none"
            />
