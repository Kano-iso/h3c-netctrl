...
const validationByLeaf = computed(() => {
  const result = new Map()
  for (const item of overview.value.latest_validation || []) {
    if (!result.has(Number(item.device_id))) result.set(Number(item.device_id), item)
  }
  return result
})
      ...
      <section class="atlas-pane">
        <template v-if="selectedVpc">
          ...
          <section v-if="workspaceMode === 'strata'" class="perspective-panel strata-view">
            <header class="perspective-heading"><div><p class="eyebrow">STRATA / DEPENDENCY</p><h3>{{ t('sdn.next.strata_title') }}</h3></div><p>{{ t('sdn.next.strata_hint') }}</p></header>
            <div class="strata-stack">
              <article class="strata-layer business-layer"><span>01</span><div><small>{{ t('sdn.next.business_layer') }}</small><strong>{{ selectedVpc.name }}</strong><p>{{ selectedVpc.tenant_name }} · {{ selectedVpc.cidr }}</p></div><b class="status-chip" :class="statusMeta(selectedVpc.status).tone">{{ statusMeta(selectedVpc.status).label }}</b></article>
              <div class="dependency-line"><i></i><span>{{ t('sdn.next.depends_on') }}</span><i></i></div>
              <article class="strata-layer logic-layer"><span>02</span><div><small>{{ t('sdn.next.logic_layer') }}</small><strong>VNI {{ selectedVpc.vni }} · {{ selectedVpc.vsi_name }}</strong><p>{{ t('sdn.next.gateway') }} {{ selectedVpc.gateway_ip || '—' }}</p></div><b>{{ t('sdn.next.desired_state') }}</b></article>
              <div class="dependency-line"><i></i><span>{{ t('sdn.next.carried_by') }}</span><i></i></div>
              <div class="physical-grid">
                <article v-for="leaf in leafs" :key="leaf.id" class="strata-layer device-layer" @click="selectObject('device', leaf)"><span>03</span><div><small>{{ t('sdn.next.device_layer') }}</small><strong>{{ leaf.name }}</strong><p>{{ leaf.host }} · {{ bindingForLeaf(leaf.id).length }} {{ t('sdn.next.access_ports') }}</p></div><b class="evidence-badge" :class="statusMeta(validationByLeaf.get(leaf.id)?.validation_result).tone">{{ validationByLeaf.has(leaf.id) ? t('sdn.next.evidenced') : t('sdn.next.no_evidence') }}</b></article>
              </div>
            </div>
            <div class="truth-legend"><span><i class="desired"></i>{{ t('sdn.next.desired_state') }}</span><span><i class="observed"></i>{{ t('sdn.next.observed_state') }}</span><span><i class="inferred"></i>{{ t('sdn.next.inferred_state') }}</span></div>
          </section>
