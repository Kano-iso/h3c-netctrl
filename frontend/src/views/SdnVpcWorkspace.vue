...
const scopeExceptionOpen = ref(false)
...
const latestAssuranceRun = computed(() => assuranceRun.value || assuranceRuns.value[0] || null)
...
function scopeMeta(classification) {
  const tone = classification === 'targeted' ? 'good' : classification === 'ambiguous' ? 'warn' : 'mute'
  return { tone, label: translatedCode('scope_classification', classification, classification) }
}
function scopeReason(member) {
  return translatedCode('scope_reason', member?.reason_code, member?.reason_code)
}
function scopeExceptionLabel(exception) {
  if (!exception) return ''
  if (exception.state !== 'active') return translatedCode('scope_exception_state', exception.state, exception.state)
  return translatedCode('scope_exception_type', exception.exception_type, exception.exception_type)
}
function attentionMeta(item) {
  const tone = item?.severity === 'blocking' ? 'bad' : item?.severity === 'review' ? 'warn' : 'mute'
  return { tone, label: translatedCode('attention_severity', item?.severity, item?.severity) }
}
function attentionMember(item) {
  return (stateProjection.value.scope?.members || []).find((member) => member.device_id === item.device_id)
}
function attentionTitle(item) {
  return translatedCode('attention_category', item?.category, item?.category)
}
function attentionAction(item) {
  return translatedCode('attention_action', item?.recommended_action, item?.recommended_action)
}
...
async function loadAssurance() {
  ...
  if (!policyResult.success || !runsResult.success) {
    error.value = policyResult.error || runsResult.error || t('sdn.next.assurance_load_failed')
    return
  }
...
async function saveAssurancePolicy() {
  ...
  clearNotice()
  const result = await sdnApi.putAssurancePolicy(selectedVpcId.value, {
    enabled: Boolean(assurancePolicy.value.enabled),
    cadence: assurancePolicy.value.cadence,
    response_mode: 'observe_only',
    version: assurancePolicy.value.version,
  })
  busy.value = ''
  if (!result.success) return void (error.value = result.error || t('sdn.next.assurance_save_failed'))
  assurancePolicy.value = result.data
  message.value = t('sdn.next.assurance_saved')
}
async function runAssurance() {
  busy.value = 'assurance-run'
  clearNotice()
  const result = await sdnApi.createAssuranceRun(selectedVpcId.value, { trigger: 'manual' })
  busy.value = ''
  if (!result.success) return void (error.value = result.error || t('sdn.next.assurance_run_failed'))
  assuranceRun.value = result.data
  const runsResult = await sdnApi.listAssuranceRuns(selectedVpcId.value, { limit: 20 })
  if (runsResult.success) assuranceRuns.value = runsResult.data?.runs || []
  message.value = t('sdn.next.assurance_completed')
}
async function openAssuranceRun(run) {
  busy.value = `assurance-run-${run.id}`
  const result = await sdnApi.getAssuranceRun(selectedVpcId.value, run.id)
  busy.value = ''
  if (!result.success) return void (error.value = result.error || t('sdn.next.assurance_load_failed'))
  assuranceRun.value = result.data
}
async function handleAssuranceItem(item) {
  ...
  if (['review_scope', 'review_exception', 'repair_context'].includes(item.recommendation) && member) {
    openScopeException(member)
    return
  }
...
async function handleAttention(item) {
  ...
  if (['review_exception', 'repair_context_record', 'review_coverage', 'review_records'].includes(item.recommended_action)) {
    const member = attentionMember(item)
    if (member) openScopeException(member)
    return
  }
  const leaf = (stateProjection.value.leaves || []).find((entry) => entry.device_id === item.device_id)
  selectObject('device', deviceById(item.device_id) || leaf || { id: item.device_id, name: item.name, host: item.host })
}
function openScopeException(member) {
  clearNotice()
  scopeExceptionMember.value = member
  const exception = member.exception
  scopeExceptionForm.value = {
    exception_type: exception?.exception_type || 'intentional_exclusion',
    reason: exception?.reason || '',
    expires_at: exception?.expires_at ? exception.expires_at.slice(0, 16) : '',
  }
  scopeExceptionOpen.value = true
}
async function saveScopeException() {
  const member = scopeExceptionMember.value
  if (!member || !scopeExceptionForm.value.reason.trim()) return
  busy.value = 'scope-exception'
  const payload = {
    exception_type: scopeExceptionForm.value.exception_type,
    reason: scopeExceptionForm.value.reason.trim(),
    expires_at: scopeExceptionForm.value.expires_at || null,
  }
  const result = await sdnApi.upsertScopeException(selectedVpcId.value, member.device_id, payload)
  busy.value = ''
  if (!result.success) return void (error.value = result.error || t('sdn.next.scope_exception_save_failed'))
  scopeExceptionOpen.value = false
  await loadContext()
  message.value = t('sdn.next.scope_exception_saved')
}
async function clearScopeException() {
  const member = scopeExceptionMember.value
  if (!member) return
  busy.value = 'scope-exception'
  const result = await sdnApi.clearScopeException(selectedVpcId.value, member.device_id)
  busy.value = ''
  if (!result.success) return void (error.value = result.error || t('sdn.next.scope_exception_clear_failed'))
  scopeExceptionOpen.value = false
  await loadContext()
  message.value = t('sdn.next.scope_exception_cleared')
}
...
function transitionTone(kind) {
  if (['drift_cleared', 'evidence_gained'].includes(kind)) return 'good'
  if (kind === 'drift_detected') return 'bad'
  if (['evidence_lost', 'state_changed'].includes(kind)) return 'warn'
  return 'mute'
}
function transitionSummary(transition) {
  if (!transition || transition.kind === 'baseline_unavailable') return t('sdn.next.transition_baseline')
  const count = transition.summary?.changed_dimensions?.length || 0
  return count ? t('sdn.next.transition_changes', { count }) : t('sdn.next.transition_unchanged')
}
function transitionDimensions(transition) {
  return (transition?.dimensions || []).filter((dimension) => dimension.transition !== 'unchanged')
}
function transitionDimensionLabel(dimension) {
  if (dimension?.binding_id != null) {
    const field = translatedCode('transition_dimension', dimension.column, dimension.column)
    return t('sdn.next.transition_binding_dimension', { id: dimension.binding_id, field })
  }
  return translatedCode('transition_dimension', dimension?.dimension, dimension?.dimension)
}
...
watch(workspaceMode, async (mode) => {
  if (mode === 'guard' && selectedVpcId.value) await loadAssurance()
})
...
<template>
  <main class="next-workbench">
    ...
    <nav class="perspective-switch" :aria-label="t('sdn.next.perspective')">
      ...
      <button :class="{ active: workspaceMode === 'guard' }" @click="workspaceMode = 'guard'">
        <span>GUARD</span><small>{{ t('sdn.next.guard_desc') }}</small>
      </button>
    </nav>
      ...
      <section class="atlas-pane">
        <template v-if="selectedVpc">
          ...
          <section v-if="workspaceMode === 'strata'" class="perspective-panel strata-view">
            ...
            <section v-if="stateProjection.scope" class="scope-coverage">
              <header>
                <span><small>{{ t('sdn.next.scope_title') }}</small><strong>{{ t('sdn.next.scope_summary', { targeted: stateProjection.scope.summary?.targeted || 0, eligible: stateProjection.scope.summary?.eligible || 0 }) }}</strong><em v-if="stateProjection.scope.summary?.active_exception" class="exception-count">{{ t('sdn.next.scope_exception_count', { count: stateProjection.scope.summary.active_exception }) }}</em></span>
                <p>{{ t('sdn.next.scope_boundary') }}</p>
              </header>
              <div class="scope-member-list">
                <article v-for="member in stateProjection.scope.members || []" :key="member.device_id" class="scope-member" @click="openScopeException(member)">
                  <span><strong>{{ member.name || `#${member.device_id}` }}</strong><small>{{ member.host || '—' }}</small></span>
                  <span><b class="status-chip" :class="scopeMeta(member.classification).tone">{{ scopeMeta(member.classification).label }}</b><small>{{ scopeReason(member) }}</small><em v-if="member.exception" class="exception-badge" :class="member.exception.state">{{ scopeExceptionLabel(member.exception) }}</em></span>
                </article>
              </div>
            </section>
            <section v-if="stateProjection.attention?.items?.length" class="attention-panel">
              <header><span><small>{{ t('sdn.next.attention_title') }}</small><strong>{{ t('sdn.next.attention_summary', { total: stateProjection.attention.summary?.total || 0, blocking: stateProjection.attention.summary?.blocking || 0 }) }}</strong></span><p>{{ t('sdn.next.attention_boundary') }}</p></header>
              <div class="attention-list">
                <article v-for="item in stateProjection.attention.items" :key="item.key">
                  ...
                  <button class="text-button" @click="handleAttention(item)">{{ attentionAction(item) }}</button>
                </article>
              </div>
            </section>
                ...
                <div v-if="historyOpenDeviceId === projection.device_id" class="history-panel" @click.stop>
                  ...
                  <div v-if="historyPoints(projection.device_id).length" class="history-track">
                    <button v-for="point in historyPoints(projection.device_id)" :key="point.snapshot_id" type="button" :class="`is-${point.aggregate}`" @click="selectHistoryPoint(projection, point)">
                      <i></i><span><strong>{{ formatTime(point.collected_at) }}</strong><small>#{{ point.snapshot_id }} · {{ point.validation_result || t('sdn.next.unknown') }}<template v-if="point.correlation?.status === 'linked'"> · {{ t('sdn.next.operation_ref', { id: point.correlation.operation_id }) }}</template></small><em :class="transitionTone(point.transition_from_prior?.kind)">{{ transitionSummary(point.transition_from_prior) }}</em></span><b :class="projectionMeta(point.aggregate).tone">{{ projectionMeta(point.aggregate).label }}</b>
                    </button>
                  </div>
          ...
          <section v-if="workspaceMode === 'guard'" class="perspective-panel guard-view" :class="{ loading: assuranceLoading }">
            <header class="perspective-heading guard-heading">
              <div><p class="eyebrow">GUARD / ASSURANCE</p><h3>{{ t('sdn.next.guard_title') }}</h3></div>
              <span class="status-chip large" :class="assuranceMeta(latestAssuranceRun?.overall).tone">{{ assuranceMeta(latestAssuranceRun?.overall).label }}</span>
            </header>

            <div class="guard-command-bar">
              <span><small>{{ t('sdn.next.guard_current_target') }}</small><strong>{{ selectedVpc.name }}</strong><b>{{ selectedVpc.cidr }} · VNI {{ selectedVpc.vni }}</b></span>
              <p>{{ t('sdn.next.guard_boundary') }}</p>
              <button class="primary" :disabled="busy === 'assurance-run' || assuranceLoading" @click="runAssurance">{{ t('sdn.next.assurance_run_now') }}</button>
            </div>

            <div class="guard-layout">
              <section class="guard-result">
                <div class="guard-score">
                  <span :class="assuranceMeta(latestAssuranceRun?.overall).tone"><b>{{ latestAssuranceRun?.summary?.blocking || 0 }}</b><small>{{ t('sdn.next.assurance_blocking') }}</small></span>
                  <span><b>{{ latestAssuranceRun?.summary?.review || 0 }}</b><small>{{ t('sdn.next.assurance_review') }}</small></span>
                  <span><b>{{ latestAssuranceRun?.summary?.deferred || 0 }}</b><small>{{ t('sdn.next.assurance_deferred') }}</small></span>
                  <span><b>{{ latestAssuranceRun?.facts?.leaves?.length || 0 }}</b><small>{{ t('sdn.next.assurance_evaluated_leafs') }}</small></span>
                </div>
                <div v-if="latestAssuranceRun" class="guard-evaluation-meta">
                  <span>{{ t('sdn.next.assurance_last_run') }} <strong>{{ formatTime(latestAssuranceRun.completed_at) }}</strong></span>
                  <span>{{ latestAssuranceRun.policy_enabled ? t('sdn.next.assurance_policy_on') : t('sdn.next.assurance_policy_off') }}</span>
                </div>
                <div v-if="latestAssuranceRun?.items?.length" class="guard-recommendations">
                ...
                <div v-else class="guard-empty">
                  <strong>{{ latestAssuranceRun ? t('sdn.next.assurance_no_findings') : t('sdn.next.assurance_not_run') }}</strong>
                  <p>{{ latestAssuranceRun ? t('sdn.next.assurance_no_findings_detail') : t('sdn.next.assurance_not_run_detail') }}</p>
                </div>
              </section>

              <aside class="guard-policy">
    ...
    <Teleport to="body">
      ...
      <Teleport to="body">
        <div v-if="scopeExceptionOpen" class="modal-backdrop" @click.self="scopeExceptionOpen = false">
          <section class="modal scope-exception-modal" role="dialog" aria-modal="true">
            <header><div><p class="eyebrow">STRATA / SCOPE CONTEXT</p><h2>{{ t('sdn.next.scope_exception_title') }}</h2></div><button class="icon-button" :aria-label="t('common.close')" @click="scopeExceptionOpen = false">×</button></header>
            <form class="form-grid" @submit.prevent="saveScopeException">
              <div class="full scope-exception-target"><small>{{ t('sdn.next.device') }}</small><strong>{{ scopeExceptionMember?.name || `#${scopeExceptionMember?.device_id}` }}</strong><span>{{ scopeExceptionMember?.host || '—' }}</span></div>
              <label>{{ t('sdn.next.scope_exception_type_label') }}<select v-model="scopeExceptionForm.exception_type" name="scope_exception_type"><option value="intentional_exclusion">{{ t('sdn.next.scope_exception_type.intentional_exclusion') }}</option><option value="maintenance_pause">{{ t('sdn.next.scope_exception_type.maintenance_pause') }}</option></select></label>
              <label>{{ t('sdn.next.scope_exception_expiry') }}<input v-model="scopeExceptionForm.expires_at" name="scope_exception_expiry" type="datetime-local"></label>
              <label class="full">{{ t('sdn.next.scope_exception_reason') }}<textarea v-model="scopeExceptionForm.reason" name="scope_exception_reason" maxlength="200" required></textarea></label>
              <div class="full scope-exception-boundary"><strong>{{ t('sdn.next.scope_exception_boundary_title') }}</strong><p>{{ t('sdn.next.scope_exception_boundary') }}</p></div>
              <footer class="full"><button v-if="scopeExceptionMember?.exception" type="button" class="danger-text" :disabled="busy === 'scope-exception'" @click="clearScopeException">{{ t('sdn.next.scope_exception_clear') }}</button><button type="button" class="secondary" @click="scopeExceptionOpen = false">{{ t('common.cancel') }}</button><button class="primary" :disabled="busy === 'scope-exception' || !scopeExceptionForm.reason.trim()">{{ t('common.save') }}</button></footer>
            </form>
          </section>
        </div>
      </Teleport>
...
.scope-coverage{margin:0 0 14px;padding:12px 0;border-top:1px solid #d8e1e2;border-bottom:1px solid #d8e1e2}.scope-coverage>header{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;margin-bottom:9px}.scope-coverage>header>span{display:flex;flex-direction:column;gap:2px}.scope-coverage>header small{font-size:9px;font-weight:850;color:#708187;text-transform:uppercase}.scope-coverage>header strong{font-size:13px;color:#263f45}.scope-coverage>header p{max-width:420px;margin:0;color:#6b7b80;font-size:10px;text-align:right}.scope-member-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:6px}.scope-member-list article{display:flex;min-width:0;align-items:center;justify-content:space-between;gap:10px;padding:8px 9px;background:#fff;border-left:3px solid #c7d2d4}.scope-member-list article>span{display:flex;min-width:0;flex-direction:column;gap:2px}.scope-member-list article>span:last-child{align-items:flex-end;text-align:right}.scope-member-list strong,.scope-member-list small{overflow-wrap:anywhere}.scope-member-list strong{font-size:11px;color:#32494f}.scope-member-list small{font-size:9px;color:#738287}.scope-member-list .status-chip{font-size:9px}.scope-member{cursor:pointer}.scope-member:hover{background:#eef6f4}.exception-count{color:#176b56;font-size:9px;font-style:normal;font-weight:800}.exception-badge{margin-top:3px;padding:2px 5px;border-radius:3px;background:#e5eef0;color:#3c6068;font-size:8px;font-style:normal;font-weight:800}.exception-badge.expired,.exception-badge.invalid{background:#fff1d6;color:#875d0c}.scope-exception-modal{width:min(620px,100%)}.scope-exception-target{display:flex;align-items:baseline;gap:8px;padding:11px;background:#eef4f3;border-left:3px solid #16796b}.scope-exception-target small{color:#64777c;font-size:9px;font-weight:850;text-transform:uppercase}.scope-exception-target span{color:#6b7c81;font-size:11px}.form-grid textarea{min-height:86px;resize:vertical;border:1px solid #cbd5d8;border-radius:5px;padding:9px 10px;font:inherit}.scope-exception-boundary{padding:11px 12px;background:#fff8e9;border-left:3px solid #c08a28}.scope-exception-boundary strong{font-size:11px;color:#6f4e10}.scope-exception-boundary p{margin:4px 0 0;color:#745f38;font-size:11px;line-height:1.5}
.attention-panel{margin:0 0 14px;padding:12px;border:1px solid #d6e0e1;background:#fff}.attention-panel>header{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;margin-bottom:9px}.attention-panel>header span{display:flex;flex-direction:column}.attention-panel>header small{color:#0d7a6b;font-size:9px;font-weight:900;text-transform:uppercase}.attention-panel>header strong{margin-top:2px;font-size:12px}.attention-panel>header p{max-width:430px;margin:0;color:#6d7d82;font-size:10px;text-align:right}.attention-list{display:flex;flex-direction:column}.attention-list article{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:10px;min-height:47px;border-top:1px solid #e5eaeb}.attention-list article>span{display:flex;min-width:0;flex-direction:column}.attention-list strong{font-size:11px}.attention-list small{margin-top:2px;color:#78878b;font-size:9px}.attention-list .text-button{font-size:10px}
...
.guard-view{padding:16px;border:1px solid #d7e0e1;background:#f8faf9}.guard-command-bar{display:grid;grid-template-columns:minmax(180px,.8fr) minmax(240px,1.4fr) auto;align-items:center;gap:18px;padding:14px 0;border-bottom:1px solid #dbe3e4}.guard-command-bar>span{display:flex;flex-direction:column}.guard-command-bar small,.guard-policy small,.guard-history small{color:#0d7a6b;font-size:9px;font-weight:900;text-transform:uppercase}.guard-command-bar strong{margin:3px 0;font-size:14px}.guard-command-bar b{color:#718086;font-size:10px}.guard-command-bar p{margin:0;color:#65767b;font-size:11px;line-height:1.5}.guard-layout{display:grid;grid-template-columns:minmax(0,1fr) 250px;gap:18px;padding:18px 0}.guard-result{min-width:0}.guard-score{display:grid;grid-template-columns:repeat(4,1fr);border:1px solid #d8e1e2;background:#fff}.guard-score>span{display:flex;flex-direction:column;padding:12px;border-right:1px solid #e2e8e9}.guard-score>span:last-child{border-right:0}.guard-score b{font-size:23px;color:#263f45}.guard-score span.bad b{color:#a02e2e}.guard-score small{margin-top:2px;color:#718086;font-size:9px}.guard-evaluation-meta{display:flex;justify-content:space-between;gap:12px;padding:9px 2px;color:#708085;font-size:9px}.guard-recommendations{border-top:1px solid #dce4e5}.guard-recommendations article{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:10px;min-height:55px;border-bottom:1px solid #dfe6e7}.guard-recommendations article>span{display:flex;min-width:0;flex-direction:column}.guard-recommendations strong{font-size:11px}.guard-recommendations small{margin-top:2px;color:#748489;font-size:9px}.guard-empty{padding:36px 14px;text-align:center;background:#fff;border:1px dashed #cbd7d9}.guard-empty p{margin:6px auto 0;max-width:440px;color:#718086;font-size:11px}.guard-policy{padding-left:17px;border-left:1px solid #d9e2e3}.guard-policy>header{display:flex;flex-direction:column;margin-bottom:14px}.guard-policy>header strong{margin-top:3px;font-size:13px}.guard-policy>label:not(.toggle-row){display:flex;flex-direction:column;gap:5px;margin-top:14px;color:#53666b;font-size:10px;font-weight:750}.guard-policy select{height:38px;border:1px solid #cbd5d8;border-radius:5px;background:#fff;padding:0 9px;color:#24343a}.toggle-row{display:flex;align-items:center;justify-content:space-between;gap:12px}.toggle-row>span{display:flex;flex-direction:column}.toggle-row small{margin-top:3px;color:#7a898d;font-weight:500;text-transform:none}.toggle-row input{position:absolute;opacity:0}.toggle-row i{position:relative;flex:0 0 38px;height:21px;border-radius:11px;background:#aebabc;cursor:pointer}.toggle-row i:after{position:absolute;top:3px;left:3px;width:15px;height:15px;border-radius:50%;background:#fff;content:'';transition:.18s}.toggle-row input:checked+i{background:#16796b}.toggle-row input:checked+i:after{left:20px}.guard-policy-note{margin:10px 0 14px;padding:8px;border-left:2px solid #c18a27;background:#fff8e9;color:#75613c;font-size:9px;line-height:1.5}.guard-policy .secondary{width:100%}.guard-history{padding-top:14px;border-top:1px solid #d7e0e1}.guard-history>header{display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:8px}.guard-history>header span{display:flex;flex-direction:column}.guard-history>header strong{margin-top:2px;font-size:12px}.guard-history>header>b{color:#718086;font-size:10px}.guard-history-list{display:flex;gap:7px;overflow:auto;padding-bottom:3px}.guard-history-list button{display:grid;grid-template-columns:8px minmax(120px,1fr) auto;align-items:center;gap:8px;min-width:200px;padding:9px;border:1px solid #d9e1e3;border-radius:5px;background:#fff;text-align:left;cursor:pointer}.guard-history-list button.active{border-color:#65a89b;background:#edf7f4}.guard-history-list button>span:nth-child(2){display:flex;flex-direction:column}.guard-history-list strong{font-size:10px}.guard-history-list small{margin-top:2px;color:#77868a;font-size:8px}.guard-history-list button>b{color:#728287;font-size:9px}
