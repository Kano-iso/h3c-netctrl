        ...
        <template v-if="selectedVpc">
          ...
          <section v-if="workspaceMode === 'strata'" class="perspective-panel strata-view">
            ...
            <section v-if="stateProjection.scope" class="scope-coverage">
              <header>
                <span><small>{{ t('sdn.next.scope_title') }}</small><strong>{{ t('sdn.next.scope_summary', { targeted: stateProjection.scope.summary?.targeted || 0, eligible: stateProjection.scope.summary?.eligible || 0 }) }}</strong></span>
                <p>{{ t('sdn.next.scope_boundary') }}</p>
              </header>
              <div class="scope-member-list">
                <article v-for="member in stateProjection.scope.members || []" :key="member.device_id">
                  <span><strong>{{ member.name || `#${member.device_id}` }}</strong><small>{{ member.host || '—' }}</small></span>
                  <span><b class="status-chip" :class="scopeMeta(member.classification).tone">{{ scopeMeta(member.classification).label }}</b><small>{{ scopeReason(member) }}</small></span>
                </article>
              </div>
            </section>
