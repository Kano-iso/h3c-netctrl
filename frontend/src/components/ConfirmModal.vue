...
// 缺省值走 i18n
const finalTitle = () => props.title || t('component.confirm.default_title')
const finalConfirm = () => props.confirmText || t('component.confirm.default_confirm')
const finalCancel = () => props.cancelText || t('component.confirm.default_cancel')
</script>
      ...
      <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="close">
        ...
        <div class="relative panel w-full max-w-md shadow-2xl" role="dialog">
          <div class="px-5 py-4 border-b border-canvas-300">
            <h3 class="text-base font-semibold text-ink-900">{{ finalTitle() }}</h3>
          </div>
