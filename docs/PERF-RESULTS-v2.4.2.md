...
### 7.4 多轮验证（v2.4.2 补 2：消除单轮随机性）

**质疑**：user 问"临界点 7/6 是不是单轮现场值，不是设备真 max？"

**方案**：临界点档位（NETCONF 8 / SSH 7） × 3 轮 + 2 倍档位（NETCONF 16 / SSH 14） × 3 轮
- 每轮 30s
- 轮间 sleep 30s（设备 session 释放）
- 档间 sleep 60s（设备完全冷却）

**NETCONF 多轮结果**：

| 档位 | 轮 1 失败 | 轮 2 失败 | 轮 3 失败 | 结论 |
|---|---|---|---|---|
| 8  | 5  | 3  | 1  | 3 轮全失败，临界点 <= 7 **稳定** |
| 16 | 15 | 13 | 15 | 3 轮高失败率，**设备不能突破 8** |

**SSH 多轮结果**：

| 档位 | 轮 1 失败 | 轮 2 失败 | 轮 3 失败 | 结论 |
|---|---|---|---|---|
| 7  | 1  | 0  | 1  | 3 轮边界值（1/0/1），临界点 <= 6 **稳定** |
| 14 | 5  | 3  | 4  | 3 轮高失败率，**设备不能突破 7** |

**最终结论**（v2.4.2 收尾版）：
- NETCONF max-session = **7**（单轮 + 多轮验证稳定）
- SSH max-session = **6**（单轮 + 多轮验证稳定）
- 设备 max-session **不是单轮现场值**，是设备配置硬限制
- 8 并发 NETCONF 失败数衰减（5→3→1）= 设备 session 池复用 + 连接 close 后端口释放延迟

**多轮脚本**：
- `backend/tests/perf/scenarios/multiround_interfaces.sh`：NETCONF 8/16 × 3 轮
- `backend/tests/perf/scenarios/multiround_backup.sh`：SSH 7/14 × 3 轮
