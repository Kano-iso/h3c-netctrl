...
- STRATA 在差异投影之前独立展示 VPC 对全部合格 EVPN Leaf 的覆盖范围，区分当前覆盖、已撤回、尚未纳入和范围待确认；范围完全沿用后端稳定分类，并明确不等于健康度或漂移结论。
...
- 当前处于 S2 连续开发阶段，不以本轮要求用户做中途验收；待跨层范围、差异和多对象解释形成完整阶段体验后，再统一发起阶段验收与主分支集成。5174 模拟环境不替代生产数据库联调；S1-027 已提供真实应用栈隔离通道（合成数据，仍不连生产数据库/设备）。
