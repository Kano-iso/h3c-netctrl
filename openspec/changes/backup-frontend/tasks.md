...
- [ ] 3.1 顶部 4 个 KPI 卡片：备份总数 / 锁定数 / 总占用 / 今日新增
- [ ] 3.2 顶部"立即全量备份"按钮
- [ ] 3.3 表格：按设备分组，每设备一段（设备名 header + 备份列表）
- [ ] 3.4 每行：时间 / 大小 / 类型 / 锁定 / hash / 操作（下载/锁定/删除/回滚）
- [ ] 3.5 锁定 / 删除 / 回滚 必经 ConfirmModal
- [ ] 3.6 全量备份按钮：调用后显示结果聚合（成功 N / 失败 M）
...
- [ ] 6.1 侧边栏 Backup.vue 加载 → 列表有历史数据
- [ ] 6.2 立即全量备份（POST /api/backups）→ 6 条新记录出现
...
- [ ] 6.9 回滚 192.168.100.4 某备份 → ConfirmModal 二次确认 → 设备配置恢复
- [ ] 6.10 CMDB.vue 顶部"全量备份"按钮 → 调用成功
- [ ] 6.11 设备不可达时操作 → 中文错误显示（如拔 SSH 但保留 830 端口）

## 7. 收尾

- [ ] 7.1 commit 代码（按子模块分批）
- [ ] 7.2 VERSION-ROADMAP.md v2.2 状态：进行中 (2/3)
- [ ] 7.3 archive change

## 8. 文档

- [ ] 8.1 `openspec/specs/backup-frontend/spec.md` 规范化能力 spec
- [ ] 8.2 README "如何做手动备份" 一节（如需要）
