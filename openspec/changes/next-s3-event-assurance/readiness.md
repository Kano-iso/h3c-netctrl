...
- S3-003 对抗测试 9 条 + 完整迁移测试 16 条 = **25 passed**；直接受影响回归
  **169 passed**。
- 隔离真实应用栈 **6 passed + BOUNDARY_OK + STACK_QA_OK**；GUARD 在启用策略后，
  对 scope-exception 新增/清除各展示一条「事件」历史，前后 device-I/O 日志不增长。
...
- 未新增独立事件详情页；复用既有 GUARD 历史展示 event 触发来源，完整事件时间线仍未做。
