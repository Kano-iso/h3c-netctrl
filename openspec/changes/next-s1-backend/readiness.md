...
> **最新复审状态（2026-09-14）：S1-026 已 CODE_REVIEW_PASSED（Codex 修正后）。** Operation 解释投影已接入 NEXT PULSE/STRATA；后端 65 项、前端 62 项及 4 个浏览器流程均通过。本轮未连接真机。S1-023 凭据卫生历史结论不变：历史口令仍需在设备和运行环境中外部轮换。

> 下方长状态链保留本轮提交复审时的历史快照；当前结论以上述“最新复审状态”和 `review-response.md` 为准。

> 状态：**READY_FOR_CODE_REVIEW**（S1-005 实现 → S1-006 闭环 CR1-CR10 → S1-007 闭环 CR11-CR18 → S1-008 闭环 CR19-CR23 → S1-009 闭环 CR24-CR26 → S1-010 闭环 CR27 → S1-011 闭环 CR28 → S1-012 闭环 CR29 → S1-013 闭环 CR30-CR31 → S1-014 闭环 CR32 → S1-015 闭环 CR35 → S1-016 闭环 CR36 → S1-017 闭环 CR37 → S1-018 闭环 CR38 → S1-019 真机验证 → S1-020 首次接入自阻断修复 + 完整真机生命周期 → S1-021 真机生命周期安全 Runner 与交付口径修正 → S1-022 Runner 审计目录失效时仍必须安全清理 → S1-023 仓库凭据卫生与 ZTP 密码边界 → S1-024 CR43 ZTP 渲染凭据边界加固 → S1-026 operation 解释投影（只读）。**等待 Codex 复审**（S1-024 已 CODE_REVIEW_PASSED；S1-026 复审结论见 review-response.md）。S1-020 修复 `_l2_ready_status` 对 `vsi_up` 的无条件必需（统一 collector `vsi_up.required = bool(active/expanding 本地绑定)` 语义，无本地绑定不再阻断首个 AC；有绑定时仍保守阻断），并于 2026-09-10 在 `.5 / 192.168.100.5 / SWC / S6850 T7064P15 / LSTN` 走完一轮完整真机生命周期：VPC create/apply ✅、display+目标 RD Type-3 作用域 ✅、access preview/execute/apply ✅（GE1/0/10 service-instance 3200 + xconnect vsi 回读确认）、complete 无主机诚实 degraded/host_observed=False ✅、access withdraw ✅（GE1/0/10 回读确认撤销）、VPC withdraw ✅、兜底清理 sdn_l3vpn+vxlan global ✅、最终残留空 ✅。**S1-021/S1-022/S1-023/S1-024/S1-026 均未连接真机、无任何设备 I/O**。真实事实（S1-020 历史执行）：测试自身 VPC/access withdraw 完成后仍发现 `sdn_l3vpn` 与 VXLAN global 两个共享对象残留，由**测试外的 ops-toolkit** 精确兜底清理并最终 readback 干净（当时尚无自动 runner；不写成当时已有 runner 自动完成）。S1-021 新增唯一宿主安全 runner（`qa/run_s1_019_real_lifecycle.sh`）包装真机 pytest：默认拒绝真机、基线共享对象已存在则拒绝承担所有权、进入可写阶段后 trap 精确兜底清理 + 最终 readback、保留 pytest 退出码、cleanup/readback 失败整体失败并指示人工、本机 flock 锁防并发。S1-022 加固审计目录契约：任何设备 I/O 前可靠创建并验证审计目录可写（`umask 077` + 临时探针，失败直接退出）；baseline 成功持久化后才进入可写阶段（写失败在 pytest 前终止）；EXIT trap 中 cleanup/final readback 一律「先真实执行并捕获返回码/输出，再尽力写审计文件」——审计目录在 pytest 期间被删除/变只读/写失败不得阻止精确清理与回读，审计写入失败本身让 runner 非零 + `MANUAL-NEEDED` 但不替代 cleanup/readback；cleanup 失败不跳过 final readback。后续真机轮次一律走该 runner（裸 pytest 只是 runner 内部实现，不承担共享对象清理）。**S1-023 凭据卫生**：活跃源码不再携带真实设备默认口令——`schemas.py` 两个 ZTP password 字段去掉默认（onboard 必填 / recovery Optional(None)）；ztp-stack 三脚本（entrypoint / ztp_runtime_render / ztp_onboard_callback）密码只从 `ZTP_ADMIN_PASS` 环境或 override 注入、缺失明确 fail closed；recovery state 明文密码落盘 git 忽略目录且权限 0600、GET/POST API 响应递归脱敏（只回 `password_set`，浏览器不拿明文）；前端加载 override 密码保持空白、留空提交 null、修改需重输；活动测试口令全替换为 synthetic；ops-toolkit `debug-v24-*` 5 个无入口临时脚本已移除。**S1-026（本轮）operation 解释投影**：`GET /api/sdn/operations/{id}` 响应新增只读、additive 的 `explanation` 投影（operation 级 intent/scope_summary/safety_boundary/truth_state/headline；attempt 级 kind/result/finished/still_uncertain/evidence_basis；unit 级 category/statement/truth_kind/source/scope/observed_at/freshness），保留全部既有字段与原始 scope/evidence 原样返回；投影为 serializer 纯函数层（新增 `services/sdn_explanation.py`），不改 apply/validate/withdraw/reconcile 行为、不新增表/不迁移；`succeeded` 无观察证据只表达执行记录成功不冒充设备验证；解释失败 `_safe_explain` 降级为 unavailable 标记绝不 500。**S1-024（历史轮次）CR43 加固**：`ztp_runtime_render._atomic_write_secret` 安全原子写（临时文件创建即 0600、replace 后显式 0600、异常清理临时文件——修复重渲染把 0600 变 0644 的回归）；entrypoint.sh 渲染改纯环境注入（python3 -c 零 `$`/零单引号插值、全部 `os.environ` 读取、`ZTP_PLATFORM_LONG` 补 export、mktemp+chmod 600+mv 原子替换+trap 清理——修复特殊字符语法失败/注入与宽权限中间文件）。**历史已暴露，必须由用户在设备与 `.env` 外部轮换**（本包不改设备、不重写 Git 历史、不改 archive/ 历史 change 与历史发布记录）。
...
- S1-020 QA：`test_s1_020_adversarial.py` = **9 passed**（修复前 5 failed / 4 passed 证明 S1-019 基线无条件要求 vsi_up）；S1-016~020 + validation/access/service/migration = **74 passed / 1 skipped**；真机一轮 = **1 passed（49.9s）** 完整生命周期（见上）。
- S1-021 QA：`test_s1_021_runner_adversarial.py` = **16 passed**（stub/fake 命令：缺门禁/目标非 .5 在设备 I/O 前失败、基线共享对象已存在则拒绝且不发出 undo、pytest 成功与失败都进入 cleanup + final readback、SIGINT/SIGTERM 触发 trap、pytest 失败保留非零 / cleanup/readback 失败 runner 非零、cleanup 仅两条精确 undo 无 save/越界、并发锁第二个 runner 退出 2）；S1-016~020 + validation/access/service/migration + runner 对抗 = **90 passed / 1 skipped**（skip=reallife 默认门）。
- S1-022 QA（本轮，**未触真机**）：runner 审计目录加固 + 对抗测试新增 5 条（审计目录不可创建/不可写→设备 I/O 调用 0、pytest 未调用、非零退出；baseline 持久化失败→不进入 pytest/可写阶段；pytest 期间删除审计目录→cleanup 两条精确 undo 仍实际调用 + final readback 仍实际调用 + runner 非零提示人工；cleanup 设备命令失败 + 审计目录失败→仍执行 final readback；并发锁测试去掉恒真断言改为真实断言）；`test_s1_021_runner_adversarial.py` = **21 passed**；S1-016~020 + validation/access/service/migration + runner 对抗 = **95 passed / 1 skipped**；`openspec validate --strict` valid；manifest JSON valid；`git diff --check` clean。
- S1-023 QA（本轮，**未触真机**）：`test_s1_023_credential_hygiene.py`（新增 11 条：活动生产路径/活动测试无真实口令字面量、debug-v24-* 已移除、ztp_runtime_render/ztp_onboard_callback 缺密码 fail closed、onboard 缺密码 422、recovery state 0600 + API 脱敏）+ `test_ztp_recovery.py` 扩展（脱敏/权限/缺密码明确失败/环境注入/沿用已有）+ `test_ztp_onboard.py`（synthetic） = ZTP+卫生 **21 passed**；受影响文件 + S1-016~022 + validation/access/service/migration + runner 对抗 = **181 passed / 16 skipped**（skip 全为 integration 门控，无真机）；前端 lint + type-check + build + vitest = **62 passed**（含 ZtpRecovery 4 条：密码留空/提交 null/显式重输/无字面量）；`openspec validate --strict` valid；manifest JSON valid；`git diff --check` clean；活动源码凭据扫描（真实默认口令前缀（含截断变体））干净——仅 archive/ 历史与 `RELEASE-NOTES-v2.3.1.md` 历史发布记录按「不改写历史」保留并显式登记。
- S1-026 QA（本轮，**未触真机**）：解释投影及畸形历史 JSON 对抗测试 = **27 passed**；受影响回归 = **65 passed**；前端 lint/type-check/build、62 个组件测试、4 个 NEXT 浏览器流程通过；`openspec validate --strict` valid、manifest JSON valid、`git diff --check` clean；**本轮无任何设备 I/O**。Codex 复审修正三项诚实性/韧性问题后通过，结论见 `review-response.md`。
- S1-024 QA（本轮，**未触真机**）：`test_s1_024_credential_hygiene.py`（新增 6 条：特殊字符密码模块级与 entrypoint 全量运行级（stub dnsmasq + 真实 jinja2）均原样渲染、不执行注入；runtime 首次与覆盖重渲染后 autocfg.cfg 均 0600；os.replace 失败路径不残留 .tmp/含口令中间文件；缺 env 写盘前明确失败；entrypoint 渲染块静态断言 os.environ 读取/mktemp/chmod/mv/trap） = **6 passed**；受影响文件 + S1-023 + S1-016~022 + validation/access/service/migration + runner 对抗 = **187 passed / 16 skipped**（skip 全为 integration 门控，无真机）；`bash -n`、`openspec validate --strict` valid、manifest JSON valid、`git diff --check` clean、活动源码凭据扫描干净（仅 archive/ 历史与 `RELEASE-NOTES-v2.3.1.md` 历史发布记录保留并登记）。
