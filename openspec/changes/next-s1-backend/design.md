...
**S2-004（CR51：设备快照时间线，只读，本轮未触真机）**：抽取共享投影助手（`_load_projection_context`/`_projection_target_devices`/`_serialize_bindings`/`_serialize_deployments`/`_decode_snapshot`/`_projection_excluded_entry`/`_projection_device_dict`），既有 `state-projection` endpoint 改用同一批助手且响应行为不变；新增 `state-projection/history`：逐快照（按 snapshot id 倒序、limit 截断）复用 `build_leaf_projection`（当前 deployment/binding 目标 desired + 该历史快照观测），每点叠加 `desired.basis=current_target`/snapshot_id/collected_at/validation_result；坏快照保留为 unknown/evidence_missing 不跳过不改写；非 evpn_leaf 仅 excluded；只读零 I/O 零写库，响应只保留 S2-003 脱敏 evidence 指针；无迁移/表/采集命令/接口字段。

**S2-006（CR52：历史快照 × 操作/尝试关联，只读，本轮未触真机）**：`_snapshot_correlation` 纯函数——快照引用（operation_id/attempt_id）为 null → `unlinked`；operation 引用 dangling → `missing`；operation 的 vpc_id/device_id 与当前时间线不一致 → `mismatch`；attempt 引用 dangling → `missing`；attempt 属于另一 operation（或仅有 attempt 引用而无可核对的 operation 引用）→ `mismatch`；全部一致 → `linked` 且只返回白名单摘要（operation: id/operation_type/status/expected_host_ip/created_at/updated_at；attempt: id/kind/status/started_at/completed_at），绝不携带 request_payload_json/scope_json/idempotency_key/fingerprint/owner/evidence_json/凭据。history endpoint 先收集当前返回窗口内快照涉及的 op/attempt id 集合、一次 `IN` 批量查询，再逐点关联（无逐点 N+1）。correlation 只表达证据归属/时间相关，不宣称操作导致状态变化；端点保持零 I/O 零写库零隐式采集；无迁移/表/接口字段变更。

**S2-008（CR53：多对象变更影响投影，只读 additive，本轮未触真机）**：`sdn_explanation.py` 新增 `build_operation_impact` 纯函数——节点（vpc/device/interface/host，仅持久化字段出现、缺失 null、truth_kind=desired、source 按 id 来源区分 operation_scope|operation_record、名称只是 label；接口为 device identity + if_index 稳定复合身份 `device:{device_id}:if_index:{if_index}`，scope 缺 device 不编造全局身份）、关系（vpc targets device / device exposes interface；interface→host 按操作类型取 expects（access）|withdraws（withdraw），只表达 scope 可证明的业务范围，不称物理邻接/转发路径/因果链，legacy 不编造）、变更项（复用 attempt_data 中已计算的 explain_attempt/explain_unit 的 truth_kind/source/statement，同一 truth 系统）、safety（target_only/ambiguous_claims 复用 explain_operation 判定，未传 explanation 时用真实 attempt facts 调用既有 explain_operation，绝不另写简化判定；共享 VPC/网关不属于 terminal access/withdraw 操作目标，legacy_apply 不可证明 → null；protected_interfaces 未持久化 → null）。`sdn_access.py::_operation_to_dict` 只读组装嵌套 `explanation.impact`（`_safe_explain` 包裹、explanation 或 impact 异常时置 `{unavailable, impact_unavailable}` 且 detail 仍 200、复用已加载数据无逐 unit/attempt N+1、旧字段不删不改）；零 DB 写、零 SSH/NETCONF、零隐式采集、无迁移/表/采集命令/接口字段变更。

**S2-010（CR54：快照状态转变投影，只读 additive，本轮未触真机）**：`sdn_state_projection.py` 新增 `build_transition` 纯函数（逐维状态转变矩阵：同 status→unchanged；drifted→unknown/stale→evidence_lost、unknown/stale→drifted→drift_detected、drifted→aligned→drift_cleared、unknown/stale→aligned→evidence_gained、aligned→unknown/stale→evidence_lost、其余含 not_applicable 参与→state_changed，None 参与→state_changed）+ `baseline_unavailable_transition` 纯标记。固定维度 vsi/vsi_up/vsi_interface/l3_vni 恒出现（缺字段→null/unchanged），绑定维度只接受数据库整数 binding_id 并按该稳定身份匹配 service_instance/access_vlan，两侧取并集保证顺序稳定；每项含 dimension key/from-to status/from-to reason_code/transition kind；顶层携带 from/to snapshot_id 与 collected_at，输出脱敏 summary（changed_dimensions + 按 kind counts），desired_basis=current_target，绝不返回原始 CLI output/error/凭据。`sdn.py` history endpoint 每点只读组装 `transition_from_prior`（较新点与紧邻较旧点比较；窗口最旧点 baseline_unavailable，不拿窗口外/当前实时状态补造基线；复用同一查询窗口与已加载数据、无新查询/N+1、旧字段不删不改）；只读零 DB 写零 SSH/NETCONF 零隐式采集、无迁移/新 endpoint/表/采集命令。

**S2-012（CR55：VPC EVPN Leaf 范围覆盖投影，只读 additive，本轮未触真机）**：`sdn_state_projection.py` 新增 `build_scope_member` 纯函数（复用 `resolve_base_lifecycle` 与 `BINDING_DESIRED_STATUSES` 同一生命周期/目标绑定判定，不复制一套；分类：base present 或存在 planned/active/expanding 当前目标绑定→targeted、base absent 且无当前目标绑定→withdrawn、无 deployment/binding/snapshot 记录→not_targeted、有历史但生命周期未证明且无当前目标绑定→ambiguous；snapshot 单独不能证明 targeted，版本不匹配仍 ambiguous，failed/pending 不覆盖确定生命周期，局部 gateway_delete 不改变 base 范围）。每个 member 只返回白名单 device_id/name/host、classification、reason_code、record_sources（deployment/binding/snapshot 存在与计数）、desired_base_state、desired_binding_count。`sdn.py` state-projection endpoint 顶层只读组装 `scope`：evpn_leaf 设备一次查询枚举、deployment/binding/snapshot 各一次批量查询按 device_id 分组（复用同一行序列化器、避免逐 Leaf N+1）；summary 只计 EVPN Leaf、无 EVPN Leaf 时 counts 全 0；非 EVPN 继续只走既有 excluded；既有 leaves/excluded/aggregate 不变；零 DB 写零 SSH/NETCONF 零隐式采集、无迁移/新 endpoint/表/采集命令/接口字段变更。

**S2-014（CR56：VPC 范围例外管理，业务上下文，写操作只改数据库，本轮未触真机）**：新增模型 `SdnScopeException`（`UniqueConstraint(vpc_id, device_id, name=uq_sdn_scope_exceptions_live)`；exception_type 仅 intentional_exclusion|maintenance_pause；reason 非空≤200；expires_at 可选；version/created_at/updated_at）与迁移 013（幂等建表/唯一索引/查询索引）。sdn.py 新增 REST：GET `/vpcs/{id}/scope-exceptions`（过期项 state=expired 返回、不自动删除）、PUT `/vpcs/{id}/devices/{device_id}/scope-exception`（新建或替换：短事务 + IntegrityError 回滚重读既有行原地更新，唯一约束保证同一 VPC+device 至多一条当前记录；重复提交稳定同一逻辑状态）、DELETE 同路径（幂等清除，deleted=true/false）。成员准入：VPC/设备不存在走既有 404 语义，非 `_is_sdn_fabric_member` 明确拒绝（不按名称/platform 推断）；expires_at 必须晚于当前；reason 必填且 ≤200；i18n 新增 7 个错误键。state-projection additive：批量读例外按 device_id 附加白名单 `exception`（无例外 null）与 `summary.active_exception` 计数，不触碰 `build_scope_member` 分类事实、不改 aggregate/leaves/excluded、不从 scope 分母移除设备。写操作零 SSH/NETCONF/配置下发/状态采集/隐式部署撤回；响应脱敏不含凭据/protected_interfaces/原始配置/快照/owner/fingerprint；畸形历史数据保守降级不 500。

**S2-014-R1（Codex 复审返工，3 项阻断修复，未触真机）**：① 父对象完整性——`SdnScopeException.vpc_id/device_id` 增加 FK 指向 `sdn_vpcs.id`/`devices.id`（ondelete=CASCADE 声明），父删除后的例外子行按项目既有 SQLite 级联方式（ORM `relationship(cascade="all, delete-orphan")`，与 SdnVpc.port_bindings/deployments、Device.asset/backups 一致；本项目未开 PRAGMA foreign_keys）清理，不留 orphan；删除例外子行不向上级联父对象/history。② ISO 时区输入——`_parse_expires_at` 接受标准 Z / ±HH:MM offset / naive ISO，aware 统一 `astimezone(UTC)` 后去 tzinfo 存为 UTC naive（与项目 DateTime 一致），naive 按 UTC naive；过去时间仍明确拒绝；序列化稳定（无 offset/Z 后缀）。③ 畸形历史不能 active——`_serialize_scope_exception` 防御式校验非法 exception_type、空/超长 reason、非 datetime expires_at、非法/缺失 version（含 bool）→ 稳定返回脱敏白名单 + `state=invalid`，绝不 active、不计入 active_exception、不 500；迁移/模型新增 CHECK（type 白名单 / LENGTH(reason)>0 / version>=1）阻止新脏数据。有效/过期旧语义不变。迁移 013 同步 FK/CHECK，空库/旧库幂等升级仍通过。

**S2-016（CR57：VPC 可行动关注队列，只读 additive，未触真机）**：新增纯函数模块 `backend/app/services/sdn_attention.py`——`build_attention_items(vpc_id, members, leaves_by_device)` + `attention_projection(...)` 实现保守矩阵（8 条），逐规则生成可共存 item（key=`{vpc_id}:{device_id}:{category}` 稳定、severity/category/recommended_action 常量映射、脱敏 source_refs、exception 复用 S2-014 白名单或 null），排序 blocking→review→deferred 再 device_id/category。sdn.py state-projection endpoint 在 scope 块后复用本请求已加载的 `scope_members`（含 exception）与 `leaves`（device_id→aggregate/observed）批量组装顶层 additive `attention`（summary + items），零额外查询、零 N+1。矩阵要点：active exception 不消灭 drift 事实（confirmed_drift 仍 blocking 携带 exception）；expired/invalid exception 独立成项且与 blocking 项共存 key 不冲突；targeted+aligned 且无 expired/invalid exception 无 item；not_targeted/withdrawn+active exception → deferred coverage_deferred 保留原分类；坏字段（未知 classification/坏 aggregate/畸形 exception state/非 dict 成员）稳定降级绝不 500。无新表/迁移/写接口/设备命令；读取零 DB 写、零 SSH/NETCONF、零隐式采集。
