...
**S2-014（CR56：VPC 范围例外管理）**：业务上下文后端纵向切片——模型+迁移 013（幂等）+ REST PUT/GET/DELETE + state-projection additive `exception`/`active_exception`；唯一约束+短事务防并发双写；成员准入/过期边界/校验/脱敏/零设备 I/O；scope 事实分类与 aggregate 不受例外影响。真实 QA：聚焦 15 passed + 受影响回归 191 passed；OpenSpec strict / manifest JSON / git diff --check 通过。未提交、未推送、未触真机。

**S2-014-R1（CR56 复审返工）**：按 Codex 阻断项修复——FK+ondelete=CASCADE+父侧 ORM cascade（项目实际级联方式）且子删除不向上级联（新增 test_exception_delete_does_not_cascade_up_to_parents / test_parent_vpc_delete_cascades_exception / test_parent_device_delete_cascades_exception）；_parse_expires_at 统一 Z/offset→UTC naive（新增 test_exception_expires_tz_z_and_offset）；_serialize_scope_exception 畸形→state=invalid + DB CHECK 拒新脏数据（新增 test_exception_serializer_malformed_history_stable_invalid / test_exception_check_constraints_reject_new_dirty_data）；迁移 013 FK/CHECK 幂等（迁移测试加 PRAGMA FK + CHECK 断言）。真实 QA：聚焦 20 passed + 受影响回归 196 passed；OpenSpec strict / manifest JSON / git diff --check 通过。未提交、未推送、未触真机。
