...
**S2-010 快照状态转变投影（只读 additive，本轮未触真机）**：`sdn_state_projection.py` 新增 `build_transition` / `baseline_unavailable_transition` 纯函数；history 每点只读组装 `transition_from_prior`（较新点与紧邻较旧点比较同一 Leaf、同一 current_target 基准；窗口最旧点明确 baseline_unavailable 不补造基线）。固定维度 vsi/vsi_up/vsi_interface/l3_vni + 按数据库整数 binding_id 匹配的 service_instance/access_vlan；保守 kind 矩阵（drifted→unknown/stale 只能 evidence_lost、unknown/stale→drifted 只能 drift_detected、not_applicable 变化按 state_changed）；携带 from/to snapshot_id 与 collected_at，并给出脱敏 summary（changed_dimensions + 按 kind counts），desired_basis=current_target，不含原始 CLI output/error/凭据。复用同一查询窗口与已加载数据无新查询/N+1；零 DB 写、零 SSH/NETCONF、零隐式采集、无迁移/新 endpoint/表/采集命令，既有字段兼容。Codex 复审补齐快照身份与畸形混合 binding ID 防护后，后端受影响回归 163 passed；S2-011 前端已接入时间线摘要和逐维下钻，前端 69 个组件测试与 46 个浏览器流程通过；OpenSpec strict / manifest JSON / git diff --check 通过。

**S2-012 VPC EVPN Leaf 范围覆盖投影（只读 additive，本轮未触真机）**：`sdn_state_projection.py` 新增 `build_scope_member` 纯函数（复用 resolve_base_lifecycle / BINDING_DESIRED_STATUSES 同一判定）；state-projection 顶层 additive 增加 `scope`（members + summary），按统一 Fabric 成员准入枚举全部 EVPN Leaf，分类保守稳定（targeted/withdrawn/not_targeted/ambiguous），snapshot 单独不证明 targeted、版本不匹配仍 ambiguous、failed/pending 不覆盖生命周期、gateway_delete 不改变 base；member 只含白名单字段，不回传凭据/protected_interfaces/原始配置或快照；deployment/binding/snapshot 各一次批量查询避免逐 Leaf N+1，summary 只计 EVPN Leaf；既有 leaves/excluded/aggregate 不变，零 DB 写、零 SSH/NETCONF、零隐式采集、无迁移/新 endpoint/表/采集命令。Codex 复审修正范围枚举与写操作共用同一成员判定，并由 S2-013 接入 STRATA 覆盖带，明确范围不等于健康度。QA：`test_s2_012_scope_projection.py` = 13 passed；受影响后端回归 = 176 passed；前端 lint/type-check/build、69 个组件测试与 46 个浏览器流程通过；OpenSpec strict / manifest JSON / git diff --check 通过。

**S2-014 VPC 范围例外管理（业务上下文，写操作只改数据库，本轮未触真机）**：新增 `SdnScopeException` 模型 + 迁移 013（幂等）；REST GET/PUT/DELETE `/vpcs/{id}/...scope-exception(s)`；类型仅 intentional_exclusion|maintenance_pause，reason 非空≤200、expires_at 可选且须晚于当前，同一 VPC+device 唯一约束至多一条当前记录，清除=直接删除不级联历史；过期读取标 state=expired 不自动删除；仅 `_is_sdn_fabric_member` EVPN Leaf 可设置，VPC/设备缺失按既有 404 语义、非成员明确拒绝；state-projection additive 附加 `exception`（null 或白名单）+ `active_exception` 计数，分类/aggregate/leaves/excluded 事实语义不变；写操作零 SSH/NETCONF/配置下发/采集/隐式部署撤回，响应脱敏。QA：`test_s2_014_scope_exception.py` = 15 passed；S2-012 13 条 + S2-001/S2-004/S2-006/S2-008/S2-010 + S1-026 与直接受影响 SDN 回归 = 191 passed；OpenSpec strict / manifest JSON / git diff --check 通过。前端由 Codex 接手，未改 next-s1-workbench。
